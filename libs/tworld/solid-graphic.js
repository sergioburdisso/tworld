    ...
    this.gameIsOver = function(robs, goal, time) {if (_NO_RENDER) return;
      ...
      setTimeout(function(){
        while ( _ACTIVE_CAMERA < _CAMERA_TYPE.AROUND_GRID )
          _self.toggleCamera();
        var cameraCase = (Math.random() < 0.5 || _LOW_QUALITY_WORLD)? 0 : 1;
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTWorld Constructor Begin
  ...
  _CL_Engine.gCCDebugOutput.OnSceneLoadingComplete = function(){
  ...
  _CL_Engine.OnLoadingComplete = function(){
    ...
    if (_ENDGAME.AGENTS_LOCATION.VALUE){
      ...
      _CLN_Flag.getMaterial(0).Tex1 = _CL_Engine
                        .getTextureManager()
                        .getTexture(
                          "./copperlichtdata/flag-mark-" + (_RESULT == _GAME_RESULT.FAILURE? "red" : "green") + ".png",
                           true
                        );
      ...
      _CLN_Flag.getMaterial(0).Tex1 = _CL_Engine
                          .getTextureManager()
                          .getTexture(
                            "./copperlichtdata/flag-mark-" + (_RESULT == _GAME_RESULT.FAILURE? "red" : "green") + "-glow.png",
                             true
                          );
