  ...
  for (var len = _KNOBS_Agents.length, i=0; i < len; ++i){
    ...
    if (_KNOBS_Agents[i].program.ai){
      ...
      if (_KNOBS_Agents[i].program.javascript){
        ...
        //_AGENTS[i].END_SOURCE_CODE          = _source.onEnd.code;
...
if (_INITIAL_STATE){
  ...
  if (empty_cells_n < _NUMBER_OF_AGENTS){
    _Ready = false;
    $("#playFrame").remove();
    console.error("[fatal error] no enough room to place the robot(s).");
  }
}
