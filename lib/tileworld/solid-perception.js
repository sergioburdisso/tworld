...
this.perceptionFunction = function( environment ) /*returns a perception*/{
	...
	if (!this.Percept){
		...
		if (_SOCKET_PROGRAM_AGENT && _SOCKET_OUTPUT_FORMAT == _PERCEPT_FORMAT.XML){
			//NOTE: any changes you make to this object must be reflected in the <tw_msg.xsd> file
			this.PerceptXML = {
				header: this.Percept.header,
				data: {
					grid	:{
								_attr_rows		: _grid.rows_length,
								_attr_columns	: _grid.columns_length,
								row				: new Array(_grid.rows_length)/*[column, ...]*/
							},
					time	:{_attr_value: 0},
					score	:{_attr_value: 0},
			...
			//initializing grid
			for (var r= _grid.rows_length-1; r >= 0; --r){
				this.PerceptXML.data.grid.row[r]= {_attr_index: r, column: new Array(_grid.columns_length)}
				for (var c= _grid.columns_length-1; c >= 0; --c)
					this.PerceptXML.data.grid.row[r].column[c] = { _attr_index:c, _attr_value:null}
			} 
			...
			case _PERCEPT_FORMAT.XML:
				...
				for (var r= _pdata.grid.rows_length-1; r >= 0; --r)
					for (var c= _pdata.grid.columns_length-1; c >= 0; --c)
						_pXMLdata.grid.row[r].column[c]._attr_value = _pdata.grid.rows[r].columns[c];
				...
				_pXMLdata.builtin_knowledge.costs.move._attr_value = environment.Costs.move;
