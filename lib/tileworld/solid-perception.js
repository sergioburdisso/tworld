...
this.perceptionFunction = function( environment ) /*returns a perception*/{
	...
	if (!this.Percept){
		...
		this.Percept = {
			...
			data:{
				...
				time: 0,
				score: -1,
				rob_id: -1, // ESTO ES NUEVO, DE PRUEBA! (no esta reflejado en el xsd)
				rob: {row: -1, column: -1},
				battery: null,
		...
		if (!Tileworld.FullyObservableGrid){
			...
			_grid.total_rows = environment.Grid.length;
			_grid.total_columns = environment.Grid[0].length;
		}
	...
	if (Tileworld.FullyObservableGrid){
		for (var r= _grid.total_rows-1; r >= 0; --r)
	...
	//-> Agent's ID
	this.Percept.data.rob_id = environment.RobID;
