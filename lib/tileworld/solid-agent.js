			...
			if (!Tileworld.PerceiveEveryTick || _READY_FOR_NEXT_ACTION){
				...
				if (!this.FirstTimeFlag){
					...
					if (percept.agent.id == my_team.leader){
						this.goal = {row:random(_ROWS - 2), column:random(_COLUMNS)};
...
function isValidMove(move){
	...
	return (_GRID.rows[arow+r].columns[acol+c] == _GRID_CELL.EMPTY ||
			_GRID.rows[arow+r].columns[acol+c] == _GRID_CELL.TILE  || 
			_GRID.rows[arow+r].columns[acol+c] == _GRID_CELL.BATTERY_CHARGER);
}
...
function printGrid(){
	...
	for (var i=0; i < _ROWS; ++i)
		strgGrid+= "|\t"+_GRID.rows[i].columns.join("\t")+"\t|\n";
