	...
	//region Methods
		//region Public Methods
			...
			//region I/O and behavior
				//--------------------------------------------------------------------------------------> leftArrowKeyPressed
				this.leftArrowKeyPressed = function() {walk(KEY_PRESSED.Left);}

				//--------------------------------------------------------------------------------------> rightArrowKeyPressed
				this.rightArrowKeyPressed = function() {walk(KEY_PRESSED.Right);}

				//--------------------------------------------------------------------------------------> upArrowKeyPressed
				this.upArrowKeyPressed = function() {walk(KEY_PRESSED.Up);}

				//--------------------------------------------------------------------------------------> downArrowKeyPressed
				this.downArrowKeyPressed = function() {walk(KEY_PRESSED.Down);}
			...
			this.animate = function() {
				...
				if (_currentAnimation != ANIMATION.None){
					...
					if (_isItTheEndOf(CL_ANIMATION.StopWalking, 10000) ||
						...
						if (!_environment.isBatteryChargeSufficient()){
			...
			this.WalkNorth = function(deterministic){
				...
				if ( _currentAnimation == ANIMATION.None && (deterministic || _takeStochasticAction(_ACTION.NORTH)) &&
					 (!Tileworld.Battery || _environment.isBatteryChargeSufficient())
					){
					...
					if (_environment.isItAValidMovement(robLocation.Row-1, robLocation.Column)){
			...
			this.WalkEast = function(deterministic){
				...
				if ( _currentAnimation == ANIMATION.None && (deterministic || _takeStochasticAction(_ACTION.EAST)) &&
					...
					if (_environment.isItAValidMovement(robLocation.Row, robLocation.Column+1)) {
		...
		//region Private
			...
			function fillHoleIfSo(){
				if (_environment.isThereAHoleToFill())
					_gTileworld.fillHole(_environment.getCellToFill());
			}
			...
			//region Animation Functions
				...
				function Animation_cannotDoThat(no){
					...
					_environment.updateBattery(-_BATTERY_INVALID_MOVE_COST);
				}
		...
		//region User Input Handler
			...
			$(document).keydown(function(e){
				if (isValidKey(e.keyCode)){
					switch(e.keyCode){
						case _ROB_CONTROLS[_index].Left:
							_LastKeyPressedIndex = 0;
							_self.leftArrowKeyPressed();
							break;
						case _ROB_CONTROLS[_index].Right:
							_LastKeyPressedIndex = 1;
							_self.rightArrowKeyPressed();
							break;
						case _ROB_CONTROLS[_index].Up:
							_LastKeyPressedIndex = 2;
							_self.upArrowKeyPressed();
							break;
						case _ROB_CONTROLS[_index].Down:
