	...
	//region Attributes
		...
		//private:
			...
			if (_AUDIO_ENABLE){
				...
				var _sound_cant = new buzz.sound("./sounds/rob-cant.mp3");
			}
	...
	//region Methods
		//region Static methods
			...
			GraphicRob.AutocalculateSpeed = function(currentFPS){
					...
					if (GraphicRob._WalkCurrentSpeed > 5) //5 = _FloorCellSize/2
		...
		//region Public Methods
			...
			//region I/O and behavior
				...
				this.leftArrowKeyPressed = function(lastKey) {
					if (lastKey)
						_KeysPressed[_LastKeyPressedIndex = 0] = true;
				...
				this.rightArrowKeyPressed = function(lastKey) {
					if (lastKey)
						_KeysPressed[_LastKeyPressedIndex = 1] = true;
				...
				this.upArrowKeyPressed = function(lastKey) {
					if (lastKey)
						_KeysPressed[_LastKeyPressedIndex = 2] = true;
					walk(KEY_PRESSED.Up);
				}
				...
				this.downArrowKeyPressed = function(lastKey) {
					if (lastKey)
						_KeysPressed[_LastKeyPressedIndex = 3] = true;
				...
				//--------------------------------------------------------------------------------------> keyUp
				this.rechageBattery = function(noStats){
					if (!noStats)
						_environment.updateStats(_index, 0/*Battery.Restore*/);

					if (!_environment.isBatteryChargeSufficient(_index)){
						if (_index == _GET_TEAM_LEADER(_index)){
							_environment.updateBattery(_index, 1000);
							_environment.updateScore(_index);
						}else
							_gTileworld.rechageBattery(_GET_TEAM_LEADER(_index), true);
					}
				}
			...
			this.animate = function() {
					...
					switch(_currentAnimation){
						...
						case  ANIMATION.CantDoThat: //does he have to say "I can't do that" to the user?
								...
								_node.getFrameNr() == _node.getNamedAnimationInfo(CL_ANIMATION.CantWhileWalking).End)
							{
								...
								if (_ROBS[_index].CONTROLLED_BY_AI)
				...
				/*if (!_outOfBatteryFlag) */{
					...
					if (isCurrentAimation(CL_ANIMATION.Standing) &&
						!_environment.isBatteryChargeSufficient(_index)){
						// Insufficient battery charge!!
						Animation_outOfBattery();
					}
				}
		...
		//region Private
			...
			function fillHoleIfSo(){
				if (_environment.isThereAHoleToFill(_index))
					_gTileworld.fillHole(_index, _environment.getCellToFill(_index));
			}
			...
			//region Animation Functions
				...
				function Animation_cannotDoThat(no){
					...
					_environment.updateStats(_index, 1/*MovesNoOK*/);
		...
		//region User Input Handler
			...
			$(document).keydown(function(e){
				if (isValidKey(e.keyCode)){
					switch(e.keyCode){
						case _ROBS[_index].CONTROLS.Left:
							_self.leftArrowKeyPressed(true);
							break;
						case _ROBS[_index].CONTROLS.Right:
							_self.rightArrowKeyPressed(true);
							break;
						case _ROBS[_index].CONTROLS.Up:
							_self.upArrowKeyPressed(true);
							break;
						case _ROBS[_index].CONTROLS.Down:
							_self.downArrowKeyPressed(true);
							break;
						case _ROBS[_index].CONTROLS.Recharge:
							_self.rechageBattery();
					}
				}
			});
