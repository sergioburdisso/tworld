		...
		//region Private
			...
			function RobStartsThinkingAboutNextAction(){
				...
				if (!this.environment){
					this.environment = {
						...
						Battery : _environment.Battery,*/
						RobLocation : _environment.RobLocation,
						ListOfHoles : []
					}
				}

				//2) preparing the environment object before sending it to the perception function
				this.environment.Score = _environment.Score;
				this.environment.Battery = _environment.Battery;

				// List Of Holes
				var _totalHoles = _environment.ListOfHoles.getLength();
				this.environment.ListOfHoles.length = _totalHoles;
				for (var iHole, iHCells, i= 0;  i < _totalHoles; ++i){
					iHole = _environment.ListOfHoles.getItemAt(i);
					...
					if (Tileworld.Deterministic)
						this.environment.ListOfHoles[i].lifetimeLeft = iHole.getTimeRemaindingToTimeout();
				}

				//3) start running the thread whose job is to create the perception -e.i. perceptionFunction(_environment)
			...
			//region Animation Functions
				...
				function Animation_cannotDoThat(){
					...
					_environment.Battery -= _BATTERY_INVALID_MOVE_COST;
				}
