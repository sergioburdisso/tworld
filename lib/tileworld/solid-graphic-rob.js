...
function GraphicRob(CLNode, graphicTileWorld){
	//region Enums
		var ANIMATION = {	None:0, WalkEast:1, WalkWest:2,
							WalkNorth:3, WalkSouth:4, CantDoThat:5,
							TurnWest:6, TurnEast:7, TurnNorth:8,
							TurnSouth:9
						};
	...
	//region Methods
				...
				if (_currentAnimation != ANIMATION.None){
					...
					switch(_currentAnimation){
						...
						case ANIMATION.WalkWest: //does he have to walk backward?
							Animation_WalkWest();
							break;
						case ANIMATION.WalkNorth: //does he have to walk to his right?
							Animation_WalkNorth();
							break;
						case ANIMATION.WalkSouth: //does he have to walk to his left?
							Animation_WalkSouth();
							break;
		...
		//region Private
			...
			function RobStartsThinkingAboutNextAction(){
				...
				//TODO: [] crea un objeto nuevo cada vez que se llama!! ineficiente
				//call the Rob's Program Agent

				//_programAgent.postMessage(createPerseption() /*total o parcial 2 -> 2+r+2 x 2+r+2*/);
				_programAgent.postMessage([
					/*n*/_environment.isItAnEmptyCell(robLocation.Row+1, robLocation.Column)||_environment.isItATile(robLocation.Row+1, robLocation.Column)? GRID_CELL.None : GRID_CELL.Obstacle,
					/*s*/_environment.isItAnEmptyCell(robLocation.Row-1, robLocation.Column)||_environment.isItATile(robLocation.Row-1, robLocation.Column)? GRID_CELL.None : GRID_CELL.Obstacle,
					/*e*/_environment.isItAnEmptyCell(robLocation.Row, robLocation.Column+1)||_environment.isItATile(robLocation.Row, robLocation.Column+1)? GRID_CELL.None : GRID_CELL.Obstacle,
					/*w*/_environment.isItAnEmptyCell(robLocation.Row, robLocation.Column-1)||_environment.isItATile(robLocation.Row, robLocation.Column-1)? GRID_CELL.None : GRID_CELL.Obstacle
				]);
			...
			function rearrangeTileNodesOnTheFloorMatrix(theCase){
				...
				switch(theCase){
					case ANIMATION.WalkSouth: //slide to the left
						xSlide = -1;
						break;
					case ANIMATION.WalkNorth: //slide to the right
						xSlide = 1;
						break;
				...
				function WalkSouth(){
					...
					if (_currentAnimation == ANIMATION.None){
						...
						if (_node.Rot.Y	== -90){
							...
							if (_environment.slideTiles(Environment.WALK.South))
				...
				function WalkNorth(){
					...
					if (_currentAnimation == ANIMATION.None){
						...
						if (_node.Rot.Y	== 90){
							...
							if (_environment.slideTiles(Environment.WALK.North))
				...
				function WalkEast(){
					...
					if (_currentAnimation == ANIMATION.None){
						...
						if (_node.Rot.Y	== 0){
							...
							if (_environment.slideTiles(Environment.WALK.East))
				...
				function WalkWest(){
					...
					if (_currentAnimation == ANIMATION.None){
						...
						if (_node.Rot.Y	== 180){
							...
							if (_environment.slideTiles(Environment.WALK.West))
			...
			//region Animation Functions
				...
				function Animation_cannotDoThat(){
					...
					if (isCurrentAimation(CL_ANIMATION.Walk)){
						...
						//Start the animation at the same legs motion
