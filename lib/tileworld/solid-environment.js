...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		...
		//public:
			...
			this.Costs.filled_hole = 1; // time the animation takes to fill a cell hole (100 = 1 "tick"--aprox. 1 second)
		...
		//region Public
			...
			this.onLoadingCompleteCallback = function() {
				...
				for (var pos, i = _rob.length-1; i >= 0; --i){
					if (_AGENTS[i].START_POSITION){//!_AGENTS_RANDOM_START){
			...
			this.updateBattery = function(rIndex, value, restored) {
				if (value < 0){
					...
					if (_ENDGAME.BATTERY_USE.VALUE)
						_checkIfGameOver(_ENDGAME.BATTERY_USE, _rob[rIndex].Stats.Battery_Use);
				}
				...
				switch(type){
					...
					case 1:
						...
						if (_ENDGAME.BAD_MOVES.VALUE)
							_checkIfGameOver(_ENDGAME.BAD_MOVES, _rob[rIndex].Stats.MovesNotOK);
						break;
				}
			}
				...
				if (this.isAHole(farthestTile[0], farthestTile[1])){
					...
					if (_rob[rIndex].HoleBeingFilled.isFilled()){
						...
						if (_ENDGAME.FILLED_HOLES.VALUE)
							_checkIfGameOver(_ENDGAME.FILLED_HOLES, _rob[rIndex].Stats.Holes);

						_rob[rIndex].Stats.Cells++;
						if (_ENDGAME.FILLED_CELLS.VALUE)
							_checkIfGameOver(_ENDGAME.FILLED_CELLS, _rob[rIndex].Stats.Cells);
					}else{
		...
		//region Private
			...
			function robWalking(rIndex, row, column) {
				...
				if (_ENDGAME.GOOD_MOVES.VALUE)
					_checkIfGameOver(_ENDGAME.GOOD_MOVES, _rob[rIndex].Stats.MovesOK);
