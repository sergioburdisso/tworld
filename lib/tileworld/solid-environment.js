...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		//private:
			...
			var _listOfTilesToSlide	= new Array(_NUMBER_OF_ROBS);

			var _battery			= 1000;
			var _bChargerLoc		= [-1, -1]; //Battery Charger Location

			var _robLocation		= new Array(_NUMBER_OF_ROBS);

			var _holeToFill			= new Array(_NUMBER_OF_ROBS);
			var _cellFilling		= new Array(_NUMBER_OF_ROBS);
			var _holeBeingFilled	= new Array(_NUMBER_OF_ROBS);
		...
		//region Public
			//region Getters And Setters
				...
				this.getRobLocation = function(rIndex) {
					return _robLocation[rIndex];
				}

				this.getListOfHoleCells = function() { return _listOfHoles; }

				this.getCellToFill = function(rIndex) { return _cellFilling[rIndex]; }

				this.getHoleBeingFilled = function(rIndex) { return _holeBeingFilled[rIndex]; }

				this.getListOfTilesToSlide = function(rIndex) { return _listOfTilesToSlide[rIndex]; }
			...
			this.onLoadingCompleteCallback = function() {
				...
				for (var pos, i = _robLocation.length-1; i >= 0; --i){
			...
			this.isThereAHoleToFill = function(rIndex) {
				if (_holeToFill[rIndex]) {
					_holeToFill[rIndex] = false;
					return true;
				} else
					return false;
			}
			...
			this.isThereAHoleFilling = function(row /*or rIndex when column is null*/, column) {
				if (column == null)
					return _cellFilling[row/*rIndex*/][0] != -1;
				else{
					for (var r= _cellFilling.length-1; r >= 0; --r)
						if (_cellFilling[r][0] == row && _cellFilling[r][1] == column)
							return true;
			...
			this.isASlidingTile = function(rIndex, row, column) {
				for (var rt = _listOfTilesToSlide.length-1; rt >= 0; --rt)
					if (rt != rIndex && _listOfTilesToSlide[rt].contains(row, column))
						return true;
				return false;
			}
			...
			this.holeFilled = function(rIndex) {
				_cellFilling[rIndex][0] = _cellFilling[rIndex][1] = -1;
			}
			...
			this.initializeRobAt = function(robIndex, rowIndex, columnIndex) {
				...
				_robLocation[robIndex].Row = rowIndex;
				_robLocation[robIndex].Column = columnIndex;
			...
			this.robWalkingToSouth = function(rIndex) {
				robWalking(rIndex, _robLocation[rIndex].Row + 1, _robLocation[rIndex].Column);
			}
			...
			this.robWalkingToEast = function(rIndex) {
				robWalking(rIndex, _robLocation[rIndex].Row, _robLocation[rIndex].Column + 1);
			}
			...
			this.robGetLocation = function(rIndex) {
				...
				if (Tileworld.Battery)
					if (_robLocation[rIndex].Row == _bChargerLoc[0] && _robLocation[rIndex].Column == _bChargerLoc[1]){
			...
			this.slideTiles = function(rIndex, theCase) {
				...
				_listOfTilesToSlide[rIndex].removeAll(); // al terminar de moverse

				switch(theCase){
					case _ACTION.NORTH: //slide to the North
					case _ACTION.SOUTH: //slide to the South
						for(var iRow = _robLocation[rIndex].Row + xSlide;
							this.isATile(iRow, _robLocation[rIndex].Column) && !this.isASlidingTile(rIndex, iRow, _robLocation[rIndex].Column);
							iRow+=xSlide
							)
								_listOfTilesToSlide[rIndex].appendUnique([iRow, _robLocation[rIndex].Column]);
						break;
					case _ACTION.EAST: //slide to the East
					case _ACTION.WEST: //slide to the West
						for(var iColumn = _robLocation[rIndex].Column + ySlide;
							this.isATile(_robLocation[rIndex].Row, iColumn) && !this.isASlidingTile(rIndex, _robLocation[rIndex].Row, iColumn);
							iColumn+=ySlide
							)
								_listOfTilesToSlide[rIndex].appendUnique([_robLocation[rIndex].Row, iColumn]);
				}

				if (_listOfTilesToSlide[rIndex].empty())
					return false;

				farthestTile= _listOfTilesToSlide[rIndex].getItemAt(_listOfTilesToSlide[rIndex].getLength()-1);
				...
				if (!this.isAValidMovement(rIndex, farthestTile[0]+xSlide, farthestTile[1]+ySlide) && !this.isAHole(farthestTile[0]+xSlide, farthestTile[1]+ySlide)){
					_listOfTilesToSlide[rIndex].removeAll();
					return false;
				}

				farthestTile = [farthestTile[0]+xSlide, farthestTile[1]+ySlide];
				_listOfTilesToSlide[rIndex].appendUnique(farthestTile);
				...
				if (this.isAHole(farthestTile[0], farthestTile[1])){
					...
					_holeBeingFilled[rIndex] = _listOfHoles.removeHoleCell([farthestTile[0], farthestTile[1]]);

					if (_holeBeingFilled[rIndex].isFilled()){

						_updateScore(_holeBeingFilled[rIndex].Value, _holeBeingFilled[rIndex].OriginalCells, true);
						removeHoleHelpers(_holeBeingFilled[rIndex]);

					}else
						_updateScore(Tileworld.valueOfCellHoleFilled(_holeBeingFilled[rIndex].getNumberOfCellsFilled()), _holeBeingFilled[rIndex].getCellsFilled(), false);

					_cellFilling[rIndex][0]= farthestTile[0];
					_cellFilling[rIndex][1]= farthestTile[1];
					_holeToFill[rIndex] = true;
				...
				closestTile = _listOfTilesToSlide[rIndex].getItemAt(0);
				...
				_listOfTilesToSlide[rIndex].removeItemAt(0);
		...
		//region Private
			...
			function removeATile(){
				...
				if (tileCell){
					...
					for (var t= _listOfTilesToSlide.length-1; t >= 0; --t)
						if (_listOfTilesToSlide[t].remove(tileCell[0], tileCell[1]))
							break;
			...
			function robWalking(rIndex, row, column) {
				...
				if (Tileworld.Battery && (_robLocation[rIndex].Row == _bChargerLoc[0] && _robLocation[rIndex].Column == _bChargerLoc[1]) ){
					$("#battery-charge-frame").css("box-shadow", "0 0 10px rgba(87, 255, 168, 0.57)");
					_grid[_robLocation[rIndex].Row][_robLocation[rIndex].Column] = _GRID_CELL.BATTERY_CHARGER;
					_graphicTileworld.batteryChargeAnimation(true);
				}else{
					_grid[_robLocation[rIndex].Row][_robLocation[rIndex].Column] = _GRID_CELL.EMPTY;
					_listOfEmptyCells.appendUnique([_robLocation[rIndex].Row, _robLocation[rIndex].Column]);
				}

				_robLocation[rIndex].Row = row;
				_robLocation[rIndex].Column = column;

				_grid[_robLocation[rIndex].Row ][_robLocation[rIndex].Column] = _GRID_CELL.AGENT;
				_listOfEmptyCells.remove(_robLocation[rIndex].Row, _robLocation[rIndex].Column);
	...
	//region Class Constructor Logic
		...
		for (var rob= 0; rob < _NUMBER_OF_ROBS; ++rob){
			_robLocation[rob] = {Row: -1, Column: -1};
			_listOfTilesToSlide[rob] = new ListOfPairs(Math.max(rows,columns));
			_cellFilling[rob] = [-1,-1];
			_holeToFill[rob]= false;
		}
