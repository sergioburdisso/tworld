	...
	//region Methods
		//region Public
			...
			this.newHole = function(holeCells, holeLifetime, actualVariabilityOfUtility) {
				var newHole = new Hole(holeCells, holeLifetime, _holeFilling, actualVariabilityOfUtility);
			...
			this.slideTiles = function(theCase) {
				...
				if (this.isItAHole(farthestTile[0], farthestTile[1])){
					...
					if (hole.isFilled()){
						...
						_updateScore(hole.Value, hole.OriginalCells, true);
						removeHoleHelpers(hole);

					}else
						_updateScore(Tileworld.valueOfCellHoleFilled(hole.getNumberOfCellsFilled()), hole.getCellsFilled(), false);
				...
				_self.Battery -= _BATTERY_SLIDE_COST;
				return true;
			}
		...
		//region Private
			...
			function _programAgentPerceive(){
				...
				this.environment.Battery = _self.Battery;
			...
			function robWalking() {
				...
				_self.Battery -= _BATTERY_WALK_COST;
			}
