...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		...
		//public:
			...
			this.Costs = {};
			this.Costs.moves = 0; // time it takes the agent to move (in "ticks") 
			this.Costs.invalid_action = 100; // wasted time by the agent when it chooses and invalid action (100 = 1 "tick"--aprox. 1 second)
			this.Costs.hole_filling = 100; // time the animation takes to fill a cell hole (100 = 1 "tick"--aprox. 1 second)
				...
				if (!_SOCKET_PROGRAM_AGENT)
					...
					switch(_SOCKET_OUTPUT_FORMAT){
						...
						case _PERCEPT_FORMAT.XML:
							percept = _X2JS.json2xml_str(percept);
							break;
