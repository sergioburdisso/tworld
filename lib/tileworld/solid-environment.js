...
function Environment(rows, columns, graphicEngine, parent) {
	//region Enums & Internal Classes
		GRID_CELL = {None:0, Tile:1, Obstacle:2, Hole:3, Rob:4};
		Environment.WALK = {East:0, West:1, North:2, South:3};
	//end region Enums
	//
	//region Attributes
		//private:
			var _grid = newMatrix(rows, columns);
			var _listOfHoles = new ListOfHoles(rows*columns); // rows*columns is space-inefficient but its speed-efficient (js engine don't use hashes)
			...
			var _score = 0;
		//public:
			this.RobLocation = new Object();
			this.RobLocation.Row = parseInt(_grid.length/2);
			this.RobLocation.Column = parseInt(_grid[0].length/2)
	...
	//region Methods
		//region Public
			...
			//(this function handles the event fired when the whole 3D scene is loaded, this event comes from the GraphicTileworld Class)
			this.onLoadingCompleteCallback = function() {
				_parent.createHoles();
				_parent.createObstacles();
			...
			this.tick = function() {
				...
				if (!this.holesSeconds){
					this.holesSeconds = uncertaintyMaker(_parent.Dynamism, _parent.Dynamism_UncertaintyThreshold) + _TELEPORT_DELAY;
					this.obstaclesSecons = uncertaintyMaker(_parent.Hostility, _parent.Hostility_UncertaintyThreshold);
				}
			...
			this.isItAHole= function(rowIndex, columnIndex) {
				return validCoordinates(rowIndex,columnIndex)&&(_grid[rowIndex][columnIndex] == GRID_CELL.Hole);
			}
			...
			this.newHole = function(holeCells, holeLifetime) {
				...
				for(var i= 0; i < holeCells.getLength(); i++){
					...
					_grid[holeCells.getItemAt(i)[0]][holeCells.getItemAt(i)[1]] = GRID_CELL.Hole;
					_listOfEmptyCells.remove(holeCells.getItemAt(i));
				}

				_listOfHoles.shrinkHoles(holeCells);
				_listOfHoles.append(new Hole(holeCells, holeLifetime, _holeFilling));
			...
			this.slideTiles = function(theCase) {
				...
				switch(theCase){
					case Environment.WALK.North: //slide to the North
						xSlide = 1;
						...
						break;
					case Environment.WALK.South: //slide to the South
						xSlide = -1;
						...
						break;
					case Environment.WALK.East: //slide to the East
						ySlide = 1;
						...
						break;
					case Environment.WALK.West: //slide to the West
						ySlide = -1;
				...
				if (this.isItAHole(farthestTile[0], farthestTile[1])){
					...
					if (hole.isFilled()){
						...
						_updateScore(_parent.utilityFillHole(hole.Size), hole.OriginalCells, true);
						removeHoleHelpers(hole);

					}else
						_updateScore(_parent.utilityFillCell(hole.getNumberOfCellsFilled()), hole.getCellsFilled(), false);
		...
		//region Private
						...
						if (_parent.HardBounds)
							...
							if (hole) {
								...
								hole.LifeTime = uncertaintyMaker(
									_parent.Dynamism,
									_parent.Dynamism_UncertaintyThreshold
								);
							}
						}
					}
			...
			function _updateScore(utility, cells, filled) {
				...
				_graphicTileworld.updateScore(_score, cells, filled, utility*10);
			}
	...
	//region Class Constructor Logic
		...
		this.initializeRobAt(this.RobLocation.Row, this.RobLocation.Column);
...
function Hole(holeCells, holeLifetime, holeFillingPointer) {
	...
	this.CurrentCells = holeCells; //ListOfPairs
	this.LifeTime = holeLifetime;
