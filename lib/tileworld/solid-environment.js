	...
	//region Methods
		//region Public
			...
			this.startRunning = function(){
				...
				if (_INITIAL_STATE){
					...
					for (id in _INITIAL_STATE.holes)
						_self.newHole(_INITIAL_STATE.holes[id],0,1-Math.random()*Tileworld.VariabilityOfUtility);
				}else{
			...
			this.onLoadingCompleteCallback = function() {
				...
				for (var pos, i = _rob.length-1; i >= 0; --i){
					if (_ROBS[i].START_POSITION){//!_ROBS_RANDOM_START){
						this.initializeRobAt(i, _ROBS[i].START_POSITION.ROW, _ROBS[i].START_POSITION.COLUMN);
			...
			this.tick = function() {
				if (_Running){
					...
					var timeLimit = _GAMEOVER.TIME.VALUE;
			...
			this.updateBattery = function(rIndex, value, restored) {
				if (value < 0){
					_rob[rIndex].Stats.Battery_Use-= value;

					if (_GAMEOVER.BATTERY_USE.VALUE)
						_checkIfGameOver(_GAMEOVER.BATTERY_USE, _rob[rIndex].Stats.Battery_Use);
				}
				...
				_graphicTileworld.updateBattery(rIndex, _rob[rIndex].Battery, restored);
			}
				...
				switch(type){
					case 0:
						...
						if (_GAMEOVER.BATTERY_RESTORE.VALUE)
							_checkIfGameOver(_GAMEOVER.BATTERY_RESTORE, _rob[rIndex].Stats.Battery_Restore);
						break;
					case 1:
						_rob[rIndex].Stats.MovesNotOK++;
						if (_GAMEOVER.MOVESNOTOK.VALUE)
							_checkIfGameOver(_GAMEOVER.MOVESNOTOK, _rob[rIndex].Stats.MovesNotOK);
						break;
				}
			}
			...
			this.robGetLocation = function(rIndex) {
				...
				if (Tileworld.Battery){
					...
					if (bcIndex){
						...
						if (_GAMEOVER.BATTERY_RECHARGE.VALUE)
							_checkIfGameOver(_GAMEOVER.BATTERY_RECHARGE, _rob[rIndex].Stats.Battery_Recharge);
					}
				}
				...
				if (_GAMEOVER.ROBS_LOCATION.VALUE){
					...
					if (_GAMEOVER.ROBS_LOCATION.ACHIEVED){
						_GAMEOVER.ROBS_LOCATION.ACHIEVED = false;
						$("#"+_GAMEOVER.ROBS_LOCATION.$ID).prop("src", "imgs/goal.png");
					}
				}
			}
		...
		//region Private
			...
			this.programAgentPerceive = function(rIndex){
				if (_Running && (Tileworld.Percept || _percept.header == _PERCEPT_HEADER.START)){
				...
				if (_Running){
					...
					while (r--)
						if (_ROBS[r].CONTROLLED_BY_AI){
							...
							_percept.data = goal.OUTCOME;
							_rob[r].ProgramAgent.send( _percept );
						}

					_graphicTileworld.gameIsOver(_rob, goal, _time);
				}
			}
		...
		for (var irob= 0; irob < _NUMBER_OF_ROBS; ++irob){
			...
			if (_ROBS[irob].CONTROLLED_BY_AI || _ROBS[irob].SOCKET_PROGRAM_AGENT){
				_rob[irob].ProgramAgent = new ProgramAgent(
					irob,
					_ROBS[irob].SOCKET_PROGRAM_AGENT,
					"./lib/tileworld/solid-agent.js",
					this,
					_graphicTileworld
				);
...
function ProgramAgent(rIndex, isSocket, src, _env, _gtw){
	...
	_programAgent.onerror = function(event){
		if (isSocket)
			console.error((_NUMBER_OF_ROBS > 1? "rob "+_index+": " : "")+"An error occurred while trying to connect to the remote Program Agent");
		//else
	}

	_programAgent.onclose = function(event) {
		console.error((_NUMBER_OF_ROBS > 1? "rob "+_index+": " : "")+"connection was closed with code: " + event.code + "\n(can't connect to the tileworld proxy)");
	}
}
