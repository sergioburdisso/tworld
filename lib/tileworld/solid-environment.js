		...
		//private:
			...
			if (_CONTROLLED_BY_AI){
				...
				if (_SOCKET_PROGRAM_AGENT){
					var _programAgent = new WebSocket("ws://localhost:"+_SOCKET_PORT);
					...
					_programAgent.onerror = function(event){
						console.error("An error occurred while trying to connect to the remote Program Agent: "+event.data);
					}
		...
		//region Private
			...
			function _perceptCreated(percept){
				...
				if (!_SOCKET_PROGRAM_AGENT)
					...
					_programAgent.send( JSON.stringify(percept) );
				}
			}
				...
				switch (parseInt(action)){
					...
					default:
						if (typeof(action) === "string"){
							if (action.toUpperCase().substr(0, _ACTION.ERROR.length) == _ACTION.ERROR)
								console.error(action.substr(_ACTION.ERROR.length));
							else
							if (action.toUpperCase().substr(0, _ACTION.MESSAGE.length) == _ACTION.MESSAGE)
								console.log(action.substr(_ACTION.MESSAGE.length));
						}
				}
			}
			//--------------------------------------------------------------------------------------> _holesAndObstaclesTick
