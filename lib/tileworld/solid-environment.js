...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		//private:
			...
			if (_ROBS[0].CONTROLLED_BY_AI || _ROBS[0].SOCKET_PROGRAM_AGENT){//LA CLAVE ES ESTO, ESTO DEBERIA INIZIALIZARSCE ABAJO JUNTO CON LOS ROBS
				...
				if (_ROBS[0].SOCKET_PROGRAM_AGENT){
					var _programAgent = new WebSocket("ws://"+_ROBS[0].SOCKET_PROGRAM_AGENT.ADDR+":"+_ROBS[0].SOCKET_PROGRAM_AGENT.PORT);
					
					if (_ROBS[0].SOCKET_PROGRAM_AGENT.OUTPUT_FORMAT == _PERCEPT_FORMAT.XML)
					var _X2JS = new X2JS();
	...
	//region Methods
		//region Public
			...
			this.startRunning = function(){
				...
				if ( _ROBS[0].CONTROLLED_BY_AI || _ROBS[0].SOCKET_PROGRAM_AGENT){
			...
			this.onLoadingCompleteCallback = function() {
				...
				if (Tileworld.Battery){
					for (var length= _bChargerLoc.length, bc=0; bc < length; ++bc){
						if (_BATTERY_RANDOM_START)
							_bChargerLoc[bc] = _listOfEmptyCells.removeItemAt( random(_listOfEmptyCells.getLength()) );
						else{
							_bChargerLoc[bc] = [_BATTERY_START_POSITION[bc].ROW, _BATTERY_START_POSITION[bc].COLUMN];
							_listOfEmptyCells.remove( _bChargerLoc[bc][0], _bChargerLoc[bc][1] );
						}

						_grid[ _bChargerLoc[bc][0] ][ _bChargerLoc[bc][1] ] = _GRID_CELL.BATTERY_CHARGER;
						_graphicTileworld.setBatteryChargerLocation(_bChargerLoc[bc][0], _bChargerLoc[bc][1]);
					}
				}
			}
			...
			this.isBatteryChargeSufficient = function(rIndex){
				...
				if (!sufficient){
					var i = _TEAMS.length;
					while (i--)
						if (_rob[ _TEAMS[i][0] ].Battery > 0) break;
			...
			this.isABatteryCharger = function(row, column){
				for (var bc = _bChargerLoc.length-1; bc >= 0; --bc)
					if (_bChargerLoc[bc][0] == row && _bChargerLoc[bc][1] == column)
						return bc+1;
				return 0;
			}
		...
		//region Private
			...
			function _sendToProgramAgent ( percept ) {
				...
				if (!_ROBS[0].SOCKET_PROGRAM_AGENT)
					...
					if (percept.header)
					switch(_ROBS[0].SOCKET_PROGRAM_AGENT.OUTPUT_FORMAT){
			...
			function _programAgentActionReceived(action) {
				...
				if (_Running){
					...
					if ( _ACTION_REGEX.SOUTH.test(action) )
						_graphicTileworld.RobWalkSouth(0);
					else
					//case _ACTION.NORTH
					if ( _ACTION_REGEX.NORTH.test(action) )
						_graphicTileworld.RobWalkNorth(0);
					else
					//case _ACTION.EAST
					if ( _ACTION_REGEX.EAST.test(action) )
						_graphicTileworld.RobWalkEast(0);
					else
					//case _ACTION.WEST
					if ( _ACTION_REGEX.WEST.test(action) )
						_graphicTileworld.RobWalkWest(0);
					else
					//case _ACTION.NONE
					...
					if ( _ACTION_REGEX.KEY_UP.test(action) ){
						...
						switch( (matchs[3]||matchs[4]).toUpperCase() ){
							case 0:
							case "LEFT":
								_graphicTileworld.keyUp(0, 0); break;
							case 1:
							case "RIGHT":
								_graphicTileworld.keyUp(0, 1); break;
							case 2:
							case "UP":
								_graphicTileworld.keyUp(0, 2); break;
							case 3:
							case "DOWN":
								_graphicTileworld.keyUp(0, 3); break;
						}
					}else
					...
					if ( _ACTION_REGEX.KEY_DOWN.test(action) ){
						...
						switch( (matchs[3]||matchs[4]).toUpperCase() ){
							case 0:
							case "LEFT":
								_graphicTileworld.RobWalkWest(0, true); break;
							case 1:
							case "RIGHT":
								_graphicTileworld.RobWalkEast(0, true); break;
							case 2:
							case "UP":
								_graphicTileworld.RobWalkNorth(0, true); break;
							case 3:
							case "DOWN":
								_graphicTileworld.RobWalkSouth(0, true); break;
						}
					}
					else
						noMoves = true;
			...
			//--------------------------------------------------------------------------------------> _updateScore
			function _updateScore(rIndex, value, cells, filled) {
				rIndex = _GET_TEAM_LEADER(rIndex);

				_rob[rIndex].Score+= value;

				if (_rob[rIndex].Score < 0)
					_rob[rIndex].Score = 0;

				_graphicTileworld.updateScore(rIndex, _rob[rIndex].Score, cells, filled, value);
			}

			//--------------------------------------------------------------------------------------> _updateScore
			function _updateScoreBattery(rIndex, bcIndex, points, calledByTimer){
				_updateScore(rIndex, -points);
	...
	//region Class Constructor Logic
		...
		for (var iteam=_TEAMS.length-1; iteam >= 0; --iteam)
			if (_TEAMS[iteam].length > maxTeamPlayers)
				maxTeamPlayers = _TEAMS[iteam].length;
