...
* [once percept is created it's returned in] (3) _perceptCreated [it calls the Program Agent (agent.js), runs its logic and] 
...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		//private:
			...
			if (_CONTROLLED_BY_AI){
				...
				var _ACTION_REGEX = {
					...
					CONSOLE_CLEAR:	/^\s*(action\s*\()?\s*(clear|console_clear|5)\s*\)?\s*$/i,
					ERROR:			/^\s*(error|console_error)\s*(\(\s*'([^]*)'\s*\)|:([^]*))\s*$/i,
					MESSAGE:		/^\s*(message|log|console_log)\s*(\(\s*'([^]*)'\s*\)|:([^]*))\s*$/i
				};
				...
				if (_SOCKET_PROGRAM_AGENT){
					...
					_programAgent.onclose = function(event) {
						console.error("WebSocket was closed with code: " + event.code + "\nReason: " + event.reason);
					}
		...
		//public:
			...
			this.Durations = {};
			this.Durations.moves = 0; // time it takes the agent to move (in "ticks") 
			this.Durations.invalid_action = 1; // wasted time by the agent when it chooses and invalid action (in "ticks")
			this.Durations.hole_filling = 1; // time the animation takes to fill a cell hole (in "ticks")
	...
	//region Methods
		//region Public
			...
			this.startRunning = function(){
					...
					if (_CONTROLLED_BY_AI){
						_perceptionFunction.onmessage = _perceptCreated;

						if (_SOCKET_PROGRAM_AGENT){
							_programAgent.send("START");
							...
							_programAgent.postMessage("START");
						}
					}
				}
			...
			this.askForNextAction = function() {
				if (Tileworld.perceiveEveryTick){
					if (_SOCKET_PROGRAM_AGENT)
						_programAgent.send("READY_FOR_NEXT_ACTION");
					else
						_programAgent.postMessage("READY_FOR_NEXT_ACTION");
				}else
			...
			this.onLoadingCompleteCallback = function() {
				...
				if (Tileworld.Battery){
					...
					_grid[ _bChargerLoc[0] ][ _bChargerLoc[1] ] = GRID_CELL.BATTERY_CHARGER;
			...
			this.tick = function() {
				if (Tileworld.Dynamic && _Running){
					...
					if (!this.holesSeconds){
						this.holesSeconds = uncertaintyMaker(Tileworld.Dynamism, Tileworld.Dynamism_UncertaintyThreshold) + _TELEPORT_DELAY;
			...
			this.isItAValidMovement= function(row, column) {
				return validCoordinates(row,column)&&(_grid[row][column] == GRID_CELL.EMPTY || _grid[row][column] == GRID_CELL.BATTERY_CHARGER);
			}

			//--------------------------------------------------------------------------------------> isItAnEmptyCell
			this.isItAnEmptyCell= function(row, column) {
				return validCoordinates(row,column)&&(_grid[row][column] == GRID_CELL.EMPTY);
			}
			...
			this.isItATile= function(row, column) {
				return validCoordinates(row,column)&&(_grid[row][column] == GRID_CELL.TILE);
			}

			//--------------------------------------------------------------------------------------> isItAObstacle
			this.isItAObstacle= function(row, column) {
				return validCoordinates(row,column)&&(_grid[row][column] == GRID_CELL.OBSTACLE);
			}
			...
			this.initializeRobAt = function(rowIndex, columnIndex) {
				...
				_grid[rowIndex][columnIndex] = GRID_CELL.AGENT;
			...
			this.robGetLocation = function() {
				...
				if (Tileworld.Battery && (_robLocation.Row == _bChargerLoc[0] && _robLocation.Column == _bChargerLoc[1]) ){
					...
					_grid[_robLocation.Row][_robLocation.Column] = GRID_CELL.BATTERY_CHARGER;
					_graphicTileworld.batteryChargeAnimation(true);
				}else{
					_grid[_robLocation.Row][_robLocation.Column] = GRID_CELL.EMPTY;
			...
			this.slideTiles = function(theCase) {
				...
				if (this.isItAHole(farthestTile[0], farthestTile[1])){
					...
					_grid[farthestTile[0]][farthestTile[1]] = GRID_CELL.EMPTY;
				...
				if (Tileworld.Battery && (closestTile[0] == _bChargerLoc[0] && closestTile[1] == _bChargerLoc[1]) )
					_grid[closestTile[0]][closestTile[1]] = GRID_CELL.BATTERY_CHARGER;
				else{
					_grid[closestTile[0]][closestTile[1]] = GRID_CELL.EMPTY;
		...
		//region Private
			//--------------------------------------------------------------------------------------> _perceptCreated
			function _perceptCreated(percept){
			...
			function _programAgentPerceive(){
				...
				if (!this.environment){
					this.environment = {
						...
						RobLocation : _self.RobLocation,
						ListOfHoles : [],
						ListOfObstacles :[],
						Durations : _self.Durations
					}
				}
				...
				for (var iHole, iHCells, i= 0;  i < _totalHoles; ++i){
					...
					if (Tileworld.Dynamic){
						this.environment.ListOfHoles[i].timeElapsed = iHole.getTimeElapsed();

						if (Tileworld.ShowTimeLeft)
							this.environment.ListOfHoles[i].lifetimeLeft = iHole.getTimeRemaindingToTimeout();
					}
				}
				...
				for (var iObs, i= 0;  i < _totalObs; ++i){
					...
					//esto deberia ser opcional, segun quiera el usuario implementar un mecanismo para "reconocer" un objeto en el ambiente o no
					//this.environment.ListOfObstacles[i].id = iObs.Id;
					this.environment.ListOfObstacles[i].cell = iObs.Cell;

					if (Tileworld.Dynamic){
						this.environment.ListOfObstacles[i].timeElapsed = iObs.getTimeElapsed();

						if (Tileworld.ShowTimeLeft)
							this.environment.ListOfObstacles[i].lifetimeLeft = iObs.getTimeRemaindingToTimeout();
					}
				}
			...
			function _programAgentActionReceived(action) {
				...
				if ( _ACTION_REGEX.SOUTH.test(action) )
					{_graphicTileworld.RobWalkSouth();return;}
				else
				//case _ACTION.NORTH
				if ( _ACTION_REGEX.NORTH.test(action) )
					{_graphicTileworld.RobWalkNorth();return;}
				else
				//case _ACTION.EAST
				if ( _ACTION_REGEX.EAST.test(action) )
					{_graphicTileworld.RobWalkEast();return;}
				else
				//case _ACTION.WEST
				if ( _ACTION_REGEX.WEST.test(action) )
					{_graphicTileworld.RobWalkWest();return;}
				else
				...
				//case _ACTION.NONE or percept needed
				if ( _ACTION_REGEX.NONE.test(action) || (!Tileworld.perceiveEveryTick && _SOCKET_PROGRAM_AGENT))
					_programAgentPerceive();
			}
			//--------------------------------------------------------------------------------------> _holesAndObstaclesTick
			...
			function _removeHole(listOfHoleCellsIndex) {
				...
				for(var i= 0, holeCell; i < currentHoleCells.getLength(); i++){
					...
					if (_grid[holeCell[0]][holeCell[1]] != GRID_CELL.EMPTY){
			...
			function _removeObstacle(obstacleIndex) {
				...
				_grid[obstacle.Cell[0]][obstacle.Cell[1]] = GRID_CELL.EMPTY;
			...
			function removeHoleCell(holeCell, isTeleported) {
				...
				_grid[holeCell[0]][holeCell[1]] = GRID_CELL.EMPTY;
			...
			function removeATile(){
				...
				if (tileCell){
					_grid[tileCell[0]][tileCell[1]] = GRID_CELL.EMPTY;
			...
			function getRandomTile() {
				...
				for (var irow= 0; irow < _grid.length; irow++)
					for (var icolumn= 0; icolumn < _grid[0].length; icolumn++){
						if (_grid[irow][icolumn] == GRID_CELL.TILE && !(_cellFilling[0] == irow && _cellFilling[1] == icolumn))
			...
			//--------------------------------------------------------------------------------------> robWalking
			function robWalking() {
				_grid[_robTargetLocation.Row ][_robTargetLocation.Column] = GRID_CELL.AGENT;
	...
	//region Class Constructor Logic
		...
		for (var irow= 0; irow < _grid.length; irow++)
			for (var icolumn= 0; icolumn < _grid[0].length; icolumn++){
				_grid[irow][icolumn] = GRID_CELL.EMPTY;
