...
function Environment(rows, columns, graphicEngine, parent) {
	//region Attributes
		//private:
			...
			var _robTargetLocation = new Array(_NUMBER_OF_ROBS);
	...
	//region Methods
		//region Public
			...
			this.initializeRobAt = function(robIndex, rowIndex, columnIndex) {
				...
				_robLocation[robIndex].Row = _robTargetLocation[robIndex].Row = rowIndex;
				_robLocation[robIndex].Column = _robTargetLocation[robIndex].Column = columnIndex;

				_graphicTileworld.setRobLocation(robIndex, rowIndex, columnIndex);
			}

			//--------------------------------------------------------------------------------------> robWalkingToSouth
			this.robWalkingToSouth = function(rIndex, listOfTilesToSlide) {
				_robTargetLocation[rIndex].Row++;
				robWalking(rIndex);
			}

			//--------------------------------------------------------------------------------------> robWalkingToNorth
			this.robWalkingToNorth = function(rIndex, listOfTilesToSlide) {
				_robTargetLocation[rIndex].Row--;
				robWalking(rIndex);
			}

			//--------------------------------------------------------------------------------------> robWalkingNorth
			this.robWalkingToEast = function(rIndex, listOfTilesToSlide) {
				_robTargetLocation[rIndex].Column++;
				robWalking(rIndex);
			}

			//--------------------------------------------------------------------------------------> robWalkingSouth
			this.robWalkingToWest = function(rIndex, listOfTilesToSlide) {
				_robTargetLocation[rIndex].Column--;
				robWalking(rIndex);
			}
			...
			this.robGetLocation = function(rIndex) {
				...
				_robLocation[rIndex].Row = _robTargetLocation[rIndex].Row;
				_robLocation[rIndex].Column = _robTargetLocation[rIndex].Column;
			...
			this.slideTiles = function(rIndex, theCase) {
				...
				switch(theCase){
					case _ACTION.SOUTH: //slide to the South
						...
						for(var iRow = _robLocation[rIndex].Row+1; this.isItATile(iRow, _robLocation[rIndex].Column); iRow++)
							_listOfTilesToSlide.appendUnique([iRow, _robLocation[rIndex].Column]);
					...
					case _ACTION.EAST: //slide to the East
						...
						for(var iColumn = _robLocation[rIndex].Column+1; this.isItATile(_robLocation[rIndex].Row, iColumn); iColumn++)
							_listOfTilesToSlide.appendUnique([_robLocation[rIndex].Row, iColumn]);
		...
		//region Private
			...
			//--------------------------------------------------------------------------------------> robWalking
			function robWalking(rIndex) {
				_grid[_robTargetLocation[rIndex].Row ][_robTargetLocation[rIndex].Column] = _GRID_CELL.AGENT;
				_listOfEmptyCells.remove(_robTargetLocation[rIndex].Row, _robTargetLocation[rIndex].Column);
	...
	//region Class Constructor Logic
		...
		for (var rob= 0; rob < _NUMBER_OF_ROBS; ++rob){
			...
			_robTargetLocation[rob]  = {Row: -1, Column: -1};
		}
