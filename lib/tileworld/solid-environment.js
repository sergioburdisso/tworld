	...
	//region Methods
		//region Public
			...
			this.tick = function() {
				if (_Running){
					...
					if (this.Time + 1 >= _TIME_LIMIT)
						_gameIsOver();
					_graphicTileworld.updateTime(_TIME_LIMIT - (++this.Time));
				}
			}
			...
			this.updateStats = function(rIndex, type){
				switch(type){
					case 0:
						_rob[rIndex].Stats.Battery.Restore++;break;
					case 1:
						_rob[rIndex].Stats.MovesNoOK++;break;
				}
			}
		...
		//region Private
			...
			this.programAgentPerceive = function(rIndex){
				if (Tileworld.Percept || _percept.header == _PERCEPT_HEADER.START){
					...
					this.environment.Score = _rob[rIndex].Score;
					this.environment.Battery = _rob[rIndex].Battery;
			...
			this.sendTeamMessage = function(team, data){
				...
				for (var ipeer = team.length-1; ipeer >= 0; --ipeer)
					if (_rob[team[ipeer]].ProgramAgent)
						_rob[team[ipeer]].ProgramAgent.send( _percept );
			}
			...
			function _gameIsOver() {
				_Running = false;
				_GameOver = true;
				_graphicTileworld.gameIsOver(_rob);
			}
