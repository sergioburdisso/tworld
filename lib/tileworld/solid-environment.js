		...
		//region Public
			...
			this.startRunning = function(){
				...
				if ( _CONTROLLED_BY_AI ){
					...
					if ( _SOCKET_PROGRAM_AGENT ){
						_programAgent.send( _PERCEPT.START );
						...
						_programAgent.postMessage( _PERCEPT.START );
					}
				}
			}
			...
			this.askForNextAction = function() {
				if ( Tileworld.perceiveEveryTick ){
					if ( _SOCKET_PROGRAM_AGENT )
						_programAgent.send( _PERCEPT.READY_FOR_NEXT_ACTION );
					else
						_programAgent.postMessage( _PERCEPT.READY_FOR_NEXT_ACTION );
				}else
