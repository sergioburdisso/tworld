...
//region Methods
	//region Public
		...
		this.gameIsOver = function(robs, outcome) {
			...
			switch(outcome){
				case _GAME.WON:
					$("#black-screen").find("#title").html(_GAMEOVER.WIN_MESSAGES.SINGLEP);
					break;
				case _GAME.LOST:
					$("#black-screen").find("#title").css("color", "red");
					$("#black-screen").find("#title").html(_GAMEOVER.LOSE_MESSAGES);
					break;
				case _GAME.NEUTRAL:
		...
		this.toggleCamera = function(){
			...
			switch(_ACTIVE_CAMERA){
				case _CAMERA_TYPE.FREE_GRID:
					...
					_cameraFinalRadius = Math.max(_ROWS, _COLUMNS)*_FloorCellSize + 10;//137;
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		_CL_CameraFlyCircle.Radius = (Math.random() < 0.2)? 60 : 1915;
