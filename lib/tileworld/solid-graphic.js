...
//region Methods
	...
	//region private
		...
		//</summary>
		//------------------------------------------------------------------------------------------------------------> onAnimate
		var oldYMouse=0, oldXMouse=0, headRotY=0, fixedHeadRotY=0, yOffset=0;
		function display(CL_Scene, currentTime) {
			if (!_Paused){
				...
				if (_cameraFirtPerson){
					var rotY= _CLN_Rob[0].Rot.Y;
					var cos = Math.cos(headRotY*_DEG_PI);
					var sin = Math.sin(headRotY*_DEG_PI);
					_CLN_UFO.Rot.Y = CL3D.radToDeg(Math.atan2(_CLN_UFO.Pos.X- _CLN_FloorBase.Pos.X, _CLN_UFO.Pos.Z - _CLN_FloorBase.Pos.Z));

					//XZ plane
					_CL_ActiveCameraTarget.X = 20*sin + _CLN_Rob[0].Pos.X;
					_CL_ActiveCameraTarget.Z = 20*cos + _CLN_Rob[0].Pos.Z;

					//XZ plane
					_CL_ActiveCamera.Pos.X = 2.4*sin + _CLN_Rob[0].Pos.X;
					_CL_ActiveCamera.Pos.Z = 2.4*cos + _CLN_Rob[0].Pos.Z;
					_CL_ActiveCamera.Pos.Y = 14 + 0.1*(_CLN_UFO.Pos.Y - _UFOFlyCenter.Y);

					if (_CL_CursorControl.MouseIsDown){

						var xOffset = (_CL_CursorControl.MouseX - oldXMouse)/3;
						yOffset = (oldYMouse - _CL_CursorControl.MouseY)/3;

						if (-70 < _CL_ActiveCameraTarget.Y + yOffset && _CL_ActiveCameraTarget.Y + yOffset < 40)
							_CL_ActiveCameraTarget.Y += yOffset;

						if (relativeAngleBetween(rotY, headRotY+xOffset) > 88)
							headRotY = relative180Angle(rotY, 88);
						else
						if (relativeAngleBetween(rotY, headRotY+xOffset) < -88)
							headRotY = relative180Angle(rotY, -88);
						else
						...
						if (!fixedHeadRotY)
							fixedHeadRotY = headRotY - rotY;
						else
							headRotY = rotY + fixedHeadRotY;
					}
					...
					if (!_cameraAnimationFlag){
						if (!_cameraShakingFlag && !CL3D.equals(_CL_ActiveCamera.Pos.Y,_cameraFixedPosY)){
							...
							_cameraShakingOrg = _CLN_UFO.Pos.Y - _UFOFlyCenter.Y;
						}else{
							_CL_ActiveCamera.Pos.Y = _cameraFixedPosY + /*(4/15)*/0.26*((_CLN_UFO.Pos.Y - _UFOFlyCenter.Y) - _cameraShakingOrg);
				...
				//region fps calculation (how many time does it take the browser to render 60 frames?)
					...
					if (_framesCounter == 0){
						...
						if (_FPSAvgCounter == 0){
							_FPSAvgCounter = 2;
							_FPSSum = _self.currentFPS + parseInt(_FPS/((currentTime - _oldCurrentTime)/_FPSTime));
						}else
							_FPSSum += parseInt(_FPS/((currentTime - _oldCurrentTime)/_FPSTime));
						...
						_self.Environment.Costs.good_move = GraphicRob.AutocalculateSpeed(_self.currentFPS);
						_self.Environment.Costs.good_move = (Math.ceil(_FloorCellSize/_self.Environment.Costs.good_move)/_self.currentFPS).toFixed(2);
		...
		this.toggleCamera = function(){
			...
			switch(_ACTIVE_CAMERA){
				...
				case _CAMERA_TYPE.FIRST_PERSON:
					...
					var cos = Math.cos(angle*_DEG_PI);
					var sin = Math.sin(angle*_DEG_PI);

					//XZ plane
					_cameraTargetFinalPos.X = 20*sin + _CLN_Rob[0].Pos.X;
					_cameraTargetFinalPos.Z = 20*cos + _CLN_Rob[0].Pos.Z;
					_cameraTargetFinalPos.Y = -30;

					//XZ plane
					_cameraFinalPos.X = 2.4*sin + _CLN_Rob[0].Pos.X;
					_cameraFinalPos.Z = 2.4*cos + _CLN_Rob[0].Pos.Z;
					_cameraFinalPos.Y = 20;
		...
		for (rob= 0; rob < _NUMBER_OF_AGENTS; ++rob){
			...
			for (var iteam = _TEAMS.length-1; iteam >= 0; --iteam)
				if (_TEAMS[iteam].MEMBERS[0] == rob){
					...
					$("#robs-hud").append(
						'<span id="rob-'+rob+'" style="display: list-item; list-style-type: none; height: 36px; '+(Tileworld.Battery?'padding: 13px 4px;':'padding-bottom: 10px; padding-right: 4px;')+'">'+
							...
							'</span>' : '') +
							'<span id="score-parent" style="float:right; font-size:20px; margin-right: '+(Tileworld.Battery?'40px; margin-top: 10px':(_NUMBER_OF_AGENTS > 1?'5px':'20px'))+'; font-family: airstrike; text-shadow: 0px 0px 10px rgba(157, 248, 255, 0.66); color: rgb(255, 105, 0)">'+
