		...
		//region Methods
			//region Public
				...
				this.fillHole = function(holeCell){
					...
					var cellToBeFilled = holeCell;//[holeCell[0], holeCell[1]];
					var _CL_FloorCell = _CL_Floor[cellToBeFilled[0]][cellToBeFilled[1]];
					var originalOnAnim;

					this.Environment.getListOfTilesToSlide().remove(holeCell);

					if (CL_Tile){
						originalOnAnim = CL_Tile.OnAnimate;
						...
						CL_Tile.OnAnimate = function(CL_Scene, current){
							if (CL_Tile.getFrameNr() >= 177500){
								_self.removeTile(cellToBeFilled);
								...
								originalOnAnim.apply(this, [CL_Scene, current]);
							}
						}
					}
				}
