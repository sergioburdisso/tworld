...
function GraphicTileworld(graphicEngine, environment){
	...
	//private:
		...
		if (_AUDIO_ENABLE){
			...
			var _sound_voice_battery_danger		= [new buzz.sound("./sounds/voices/"+_LANGUAGE+"/voice_battery-danger1.mp3"), new buzz.sound("./sounds/voices/"+_LANGUAGE+"/voice_battery-danger1.mp3")];
...
//region Methods
	//region Public
		...
		this.gameIsOver = function(robs) {
			...
			$("#black-screen").find("#title").html("TIME'S UP!");
			$("#black-screen").find("#table").show();

			$("#black-screen").show();
			$("#black-screen").animate({opacity:1}, 4000);
		...
		this.updateBattery = function(rIndex, value) {
			...
			if (this.batteryWarning[rIndex] && value <= (_COLUMNS+_ROWS)*_BATTERY_WALK_COST){
				...
				if (_AUDIO_ENABLE)
					_sound_voice_battery_danger[random(_sound_voice_battery_danger.length)].play();
				_batteryWarning(2);
			}
	...
	//region private
			...
			if (_Running){
				...
				if (_Paused){
					if (_AUDIO_ENABLE){
						...
						_sound_voice_pause.setPercent(0).setVolume(100).play();
					}
					...
					$("#tileworld").addClass("blur");
					$("#header").hide();
					$("#black-screen").find("#title").html("PAUSE");
					$('#playBtn').show();
					$("#black-screen").show();
		...
		this.toggleCamera = function(){
			...
			switch(_ACTIVE_CAMERA){
				case _CAMERA_TYPE.FREE_GRID:
					...
					_cameraFinalRadius = Math.max(_ROWS, _COLUMNS)*_FloorCellSize + 20;//137;
		...
		this.showTeleport = function(node, isTile, isRed, isBattery) {
			...
			if (isBattery){
				...
				if (_AUDIO_ENABLE)
					if (!isRed){
						_sound_voice_energy[random(_sound_voice_energy.length)].play();
		...
		this.showHoleFilledLight = function(rIndex, pair /*[x,z]*/) {
			...
			if (_scoreAnimation[rIndex].Points > 0 && _Running){
				...
				$($scoreUp).html(_scoreAnimation[rIndex].Points);
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		var _CLN_Moon, _CLN_Planet, ufoLaserAim, ufoHud;
