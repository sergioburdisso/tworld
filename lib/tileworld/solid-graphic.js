...
//region Attributes
			...
			//region Public
				...
				this.newHole = function(holeCells){
					...
					for (var i = 0, laser, row, column; i < holeCells.getLength(); i++){
						...
						_CL_Floor[row][column][1].Visible = false;
						...
						_CL_HoleHelpers[row][column].Visible = _SHOW_HOLES_HELPERS;
					...
					if (CL_Tile){
						...
						CL_Tile.OnAnimate = function(CL_Scene, current){
							if (CL_Tile.getFrameNr() >= 177500){
								...
								_CL_FloorCell[0].Visible = true;
								_CL_FloorCell[1].Visible = true;
				...
				function display(CL_Scene, currentTime) {
					if (!_Pause) {
						...
						if (_CL_LaserBeams.length == 0){
							...
							_CL_Scene.getSceneNodeFromName('laserbeam-light').Visible = false;
					...
					for (var irow= 0; irow < _CL_HoleHelpers.length; irow++)
						for (var icolumn= 0; icolumn < _CL_HoleHelpers[0].length; icolumn++)
							if (_CL_HoleHelpers[irow][icolumn])
								_CL_HoleHelpers[irow][icolumn].Visible = _SHOW_HOLES_HELPERS;
				}
		...
		//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
			...
			_CL_Engine.OnLoadingComplete = function(){
				...
				_CLN_FloorCellTop.Visible = false;
				...
				_CLN_Ship.Rot.X = -30;
				_CLN_Ship.Stop = false;
				_ShipFlyCenter = _CLN_FloorBase.Pos;
				...
				_self.setRobLocation(_self.Environment.RobLocation.Row, _self.Environment.RobLocation.Column);
