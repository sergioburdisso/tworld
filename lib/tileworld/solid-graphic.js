...
//region Methods
	...
	//region private
		...
		//------------------------------------------------------------------------------------------------------------> display
		//since this function runs on every frame the code within it must be efficient, that is why
		...
		function display(CL_Scene, currentTime) {
			if (!_Paused){
				...
				if (_cameraFirtPerson){
					...
					//XZ plane
					_CL_ActiveCameraTarget.X = 20*sin + _robPosX;
					_CL_ActiveCameraTarget.Z = 20*cos + _robPosZ;

					//XZ plane
					_cameraPos.X = 2.4*sin + _robPosX;
					_cameraPos.Z = 2.4*cos + _robPosZ;
					_cameraPos.Y = 14 + 0.1*(_ufoPosY - _UFOFlyCenter.Y);
					...
					if (_CL_CursorControl.MouseIsDown){
						...
						if (relativeAngleBetween(_robRotY, headRotY+xOffset) > 88)
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		_self.Environment.Costs.good_move = (Math.ceil(_FloorCellSize/_ROB_WALKSPEED)/_FPS).toFixed(2);
