	...
	//region Public
		...
		this.gameIsOver = function(robs, goal, time) {
			...
			for (iMm in _i){
				...
				if (_i[iMm].$e)
					_i[iMm].$e.html((!/BatteryUse/i.test(iMm)? value : (value/10).toFixed(1)+"%") + ((_NUMBER_OF_ROBS > 1)? "<br>(" + _ROBS[_i[iMm].v].NAME +")" : ""));
			...
			switch(goal.OUTCOME){
				...
				case _GAME.LOST:
					...
					$("#sub-title").html(
						...
						:
						_GAMEOVER.MESSAGES.LOST.SUBTEXTS[random(_GAMEOVER.MESSAGES.LOST.SUBTEXTS)]
					);
