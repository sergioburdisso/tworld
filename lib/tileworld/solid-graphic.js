...
function GraphicTileworld(graphicEngine, environment){
	...
	//private:
		...
		var _holeFilledCells = new Array(_NUMBER_OF_ROBS); //for glowing light animation
		var _points = new Array(_NUMBER_OF_ROBS); //for score up animation
...
//region Methods
	//region Public
		...
		this.rechageBattery = function(rIndex, noStats){this.Rob[rIndex].rechageBattery(noStats)}

		this.gameIsOver = function(robs) {
			var	cameraCase, imaxCells, imaxMoves,
				iminMoves, imaxMovesNoOK, imaxScore,
			...
			for (var holes, moves, battery, i=0; i < _TEAMS.length; i++){
				...
				for (var m=_TEAMS[i].MEMBERS.length-1; m >= 0; --m){
					...
					_TEAMS[i].STATS.mMovesNoOK += robs[_TEAMS[i].MEMBERS[m]].Stats.MovesNoOK;
				...
				if (r < _TEAMS.length){
					if (finalPositions[pos] instanceof Array){// if it is a group of tied results
						for (var i= 0; i < finalPositions[pos].length && r < 3; ++i, ++r){
							...
							$('#'+(r+1)+'-moves').html(finalPositions[pos][i].STATS.mMovesOK + "/" + finalPositions[pos][i].STATS.mMovesNoOK);
						...
						$('#'+(r+1)+'-moves').html(finalPositions[pos].STATS.mMovesOK + "/" + finalPositions[pos].STATS.mMovesNoOK);
			...
			imaxMovesNoOK = imaxBatteryUse = imaxBatteryRecharge =
			imaxBatteryRestore = imaxScore = 0;

			for (var i= 1; i < _NUMBER_OF_ROBS; i++){
				if (robs[i].Stats.MovesNoOK > robs[imaxMovesNoOK].Stats.MovesNoOK)
					imaxMovesNoOK = i;
			...
			for (var i= 0; i < _NUMBER_OF_ROBS; i++){
				if (imaxMovesNoOK != -1 && imaxMovesNoOK != i && robs[i].Stats.MovesNoOK == robs[imaxMovesNoOK].Stats.MovesNoOK)
					imaxMovesNoOK = -1;
			...
			if (imaxBatteryRecharge != -1)
				...
				$('#max-battery-recharge').parent().hide();
			...
			$("#header").hide();
			$("#game-over").show();
			$("#game-over").animate({opacity:1}, 4000);

			cameraCase = (Math.random() < 0.5)? 0 : 1;
		...
		this.updateTime = function(value) {
			...
			$("#time").html((value > 10)? mins +":"+ segs : value);

			if (value <= 10){
				$("#time").css("color", "red").css("font-size","64px");
		...
		this.updateScore = function(rIndex, score, holeCells, filled, points) {
			...
			if (points > 0){
				...
				_holeFilledCells[rIndex].length = holeCells.length;
				_points[rIndex] = points;

				for (var i=0; i < holeCells.length; i++){
					_holeFilledCells[rIndex][i] = [
	...
	//region private
		...
		this.toggleCamera = function(){
			...
			switch(_ACTIVE_CAMERA){
				case _CAMERA_TYPE.FREE_GRID:
					...
					_cameraFinalPos.set(-(Math.max(_ROWS, _COLUMNS)*_FloorCellSize-40), 60, Math.max(_ROWS, _COLUMNS)*_FloorCellSize/2);
		...
		this.showHoleFilledLight = function(rIndex, pair /*[x,z]*/) {
			...
			if (_points[rIndex] > 0 && _Running){
				pos2d = _CL_Engine.get2DPositionFrom3DPosition(light.Pos);
				$($scoreUp).html(_points[rIndex]);
		...
		function createHoleFilledAnimationLight(rIndex) {
			...
			//if (!_holeFilledCells[rIndex].length) return null;

			for (var i=0, CLN_CellLight; i < _holeFilledCells[rIndex].length; i++)
				_self.showHoleFilledLight(rIndex, _holeFilledCells[rIndex][i]);

			//_holeFilledCells[rIndex].length = 0;
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		//_CLN_UFO.getMaterial(1).Type = CL3D.Material.EMT_TRANSPARENT_ADD_COLOR;7-10
		...
		var ufoLaserAim, ufoHud = _CL_Scene.getSceneNodeFromName('clouds').createClone();
		...
		ufoHud2 = ufoHud.createClone();
		ufoHud2.Scale.set(0.05,0.0,0.05);
		ufoHud2.Pos.set(0, 20, 0);
		ufoHud2.Rot.set(-30, 180, 0);

		ufoLaserAim = ufoHud2.createClone();
		...
		//ufoLaserAim.Animators.push(new CL3D.AnimatorRotation(new CL3D.Vect3d(5,5,5)));

		_CLN_UFO.addChild(ufoHud);
		//_CLN_UFO.addChild(ufoHud2);
		...
		for (rob= 0; rob < _NUMBER_OF_ROBS; ++rob){
				...
				if (_TEAMS[iteam].MEMBERS[0] == rob){
					...
					$("#robs-hud").append(
						'<span id="rob-'+rob+'" style="display: list-item; height: 36px; '+(Tileworld.Battery?'padding: 10px 4px;':'padding-bottom: 10px; padding-right: 4px;')+'">'+
							(_NUMBER_OF_ROBS > 1? '<span id="tm-'+rob+'-logo"'+((_TEAMS[iteam].MEMBERS.length > 1)? 'class="rotate90"' :'')+' style="float: right; font-family: airstrike; margin-right: 13px; margin-top: -3px; height: 40px; width: 34px; font-size: 34px; background: url(\'./copperlichtdata/tm' + rob + '.png\') no-repeat; background-size: contain; padding-right: 5px; text-align: center;">'+((_TEAMS[iteam].MEMBERS.length == 1)? rob+1 : '')+'</span>':'')+
			...
			_holeFilledCells[rob] = new Array();//for glowing light animation
...
CL3D.LaserBeam = function(srcP, destP, engine, lifeTime)
{
	...
	_CLN_LaserGlow.setVisible(true);
