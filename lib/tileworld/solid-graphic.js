	...
	//region Public
		...
		this.gameIsOver = function(robs, goal, time) {
			...
			var _i = {
				...
				maxMovesNotOK: {v: 0, $e: $('#invalid-moves')},
			...
			for (var holes, moves, battery, i=0; i < _TEAMS.length; i++){
				...
				for (var m=_TEAMS[i].MEMBERS.length-1; m >= 0; --m){
					...
					_TEAMS[i].STATS.mBattery_Use += robs[_TEAMS[i].MEMBERS[m]].Stats.Battery_Use;
		...
		function display(CL_Scene, currentTime) {
			if (!_Paused){
				...
				if (_cameraFirtPerson){
					...
					_CL_ActiveCamera.Pos.Y = 14 + 0.1*(_CLN_UFO.Pos.Y - _ShipFlyCenter.Y);
