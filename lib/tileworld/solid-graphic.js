	...
	//region Public
		...
		this.gameIsOver = function(robs) {
			var	imaxCells,
				...
				imaxBattery,
				imaxScore,
				finalPositions = new Array(3);

			//TEAMS STATS:
			for (var holes, moves, battery, i=0; i < _TEAMS.length && i < 3; i++){
				_TEAMS[i].STATS = {
			...
			// -Battery_Restore
			// -Battery_Recharge
			finalPositions = _sortAndPartition(_TEAMS).flattening();


			//1st, 2nd and 3rd positions
			for (var holes, moves, battery, i=0; i < 3; i++){
				if (i < finalPositions.length){
					$('#'+(i+1)+'-player').html(finalPositions[i].NAME);
					$('#'+(i+1)+'-score').html(finalPositions[i].STATS.MFinalScore + "/" + finalPositions[i].STATS.MTotalScore);
					$('#'+(i+1)+'-holes').html(finalPositions[i].STATS.MHoles + "/" + finalPositions[i].STATS.MCells);
					$('#'+(i+1)+'-moves').html(finalPositions[i].STATS.mMovesOK + "/" + finalPositions[i].STATS.mMovesNoOK);
					$('#'+(i+1)+'-battery').html((finalPositions[i].STATS.mBattery_Use/10).toFixed(1) + "/" + finalPositions[i].STATS.mBattery_Restore);
				}else
					$('#'+(i+1)+'-row').hide();
			}
			...
			for (var i= 1; i < _NUMBER_OF_ROBS; i++){
				...
				if (robs[i].Stats.Battery.Restore > robs[imaxBattery].Stats.Battery.Restore)
					imaxBattery = i;
			...
			for (var i= 0; i < _NUMBER_OF_ROBS; i++){
				...
				if (imaxBattery != -1 && imaxBattery != i && robs[i].Stats.Battery.Restore == robs[imaxBattery].Stats.Battery.Restore)
					imaxBattery = -1;
			...
			if (iminMoves != -1)
				$('#min-moves').html(robs[iminMoves].Stats.MovesOK + " (" + _ROBS[iminMoves].NAME +")");
			else
				$('#min-moves').parent().hide();

			if (imaxMoves != -1)
				$('#max-moves').html(robs[imaxMoves].Stats.MovesOK + " (" + _ROBS[imaxMoves].NAME +")");
			else
				$('#max-moves').parent().hide();

			if (imaxMovesNoOK != -1)
				$('#invalid-moves').html(robs[imaxMovesNoOK].Stats.MovesNoOK + " (" + _ROBS[imaxMovesNoOK].NAME +")");
			else
				$('#invalid-moves').parent().hide();

			if (imaxCells != -1)
				$('#max-cells').html(robs[imaxCells].Stats.Cells + " (" + _ROBS[imaxCells].NAME +")");
			else
				$('#max-cells').parent().hide();

			if (imaxBattery != -1)
				$('#max-battery').html(robs[imaxBattery].Stats.Battery.Restore + " (" + _ROBS[imaxBattery].NAME +")");
			else
				$('#max-battery').parent().hide();

			if (imaxScore != -1)
				$('#max-score').html(robs[imaxScore].Stats.Score + " (" + _ROBS[imaxScore].NAME +")");
			else
			...
			_CL_CameraFlyCircle.Center.setTo(_CLN_Rob[finalPositions[0].MEMBERS[0]].Pos);
			_CL_CameraFlyCircle.Center.Y += 16;
			_CL_ActiveCameraTarget.setTo(_CL_CameraFlyCircle.Center);
			_CL_CameraFlyCircle.Radius = Math.max(_ROWS, _COLUMNS)*_FloorCellSize/2;
