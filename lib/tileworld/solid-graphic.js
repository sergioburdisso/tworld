...
//region Methods
	//region Public
		...
		this.updateBattery = function(rIndex, value) {
			...
			if (value <= 0)
				$($id).find("#battery-charge").width(value/10 + "%");
			else
				$($id).find("#battery-charge").stop(true).animate({width:value/10 + "%"},300);
		}

		this.batteryChargeAnimation = function(rIndex, out){
			rIndex = _GET_TEAM_LEADER(rIndex);
			if (out){
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
					...
					$("#robs-hud").append(
						'<span id="rob-'+rob+'" style="display: list-item; height: 36px; '+(Tileworld.Battery?'padding: 10px 4px;':'padding-bottom: 10px; padding-right: 4px;')+'">'+
							(Tileworld.Battery? '<span id="battery-parent" style="float:right; margin: -3px 15px 0 -35px; text-shadow: 0px 0px 10px cyan;">'+
								...
								'</span>'+
								'<span id="battery-percent" style="float: right; margin-top: 14px; font-size: 10px; -webkit-transform: rotate(90deg); -ms-transform: rotate(90deg); transform: rotate(90deg);">100.0%</span>'+
							'</span>' : '') +
							'<span id="score-parent" style="float:right; font-size:20px; margin-right: '+(Tileworld.Battery?'40px; margin-top: 10px':'16px')+'; font-family: airstrike; text-shadow: 0px 0px 10px rgb(255, 0, 201); color: rgb(255, 105, 0)">'+
		...
		for (var icon, rob= 0; rob < _NUMBER_OF_ROBS; ++rob){
			icon = new CL3D.BillboardSceneNode();
			icon.getMaterial(0).Tex1 = _CL_Engine.getTextureManager().getTexture("./copperlichtdata/p" + _GET_TEAM_LEADER(rob) + ".png", true);
			icon.getMaterial(0).Type = CL3D.Material.EMT_TRANSPARENT_ALPHA_CHANNEL;
			icon.SizeX = icon.SizeY = 4;
			icon.Pos.set(0, 26, 0);

			_CLN_Rob[rob].addChild(icon);
		}

		_clOnAnimateCallBack = _CL_Scene.getRootSceneNode().OnAnimate;
		_CL_Scene.getRootSceneNode().OnAnimate = display;

		_self.RobWalkNorth = _self.Rob[0].WalkNorth;
		_self.RobWalkSouth = _self.Rob[0].WalkSouth;
		_self.RobWalkEast = _self.Rob[0].WalkEast;
		_self.RobWalkWest = _self.Rob[0].WalkWest;
