			...
			//region Public
				//region Getters And Setters
					...
					this.setBatteryChargerLocation = function(row, column){
						...
						_CLN_Elect = _CL_Scene.getSceneNodeFromName('obstacle').createClone(_CL_Scene.getSceneNodeFromName('dummy'));
						_CLN_Elect.Pos.X = GraphicTileworld.RowIndexToXPosition(row);
						_CLN_Elect.Pos.Z = GraphicTileworld.ColumnIndexToZPosition(column);
						_CLN_Elect.Scale.Y = 3;
						_CLN_Elect.getMaterial(0).Tex1 = _CL_Engine.getTextureManager().getTexture("./copperlichtdata/battery-elec.png", true);
				...
				this.batteryChargeAnimation = function(out){
					if (out)
						...
						_CLN_Elect.setCurrentFrame(0);
						_CLN_Elect.Scale.Y = 5.8;
					}
				}
