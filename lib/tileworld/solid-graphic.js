	...
	//region Public
		...
		this.fillHole = function(rIndex, holeCell){
			...
			if (CL_Tile){
				...
				CL_Tile.Rot.Y = to360Degrees(this.Rob[rIndex].getCLNode().Rot.Y);

				if ( 45 <= CL_Tile.Rot.Y && CL_Tile.Rot.Y < 135 )
					CL_Tile.Rot.Y = 90;
				else
					if ( 135 <= CL_Tile.Rot.Y && CL_Tile.Rot.Y < 225 )
						CL_Tile.Rot.Y = 180;
					else
						if ( 225 <= CL_Tile.Rot.Y && CL_Tile.Rot.Y < 315 )
							CL_Tile.Rot.Y = -90;
						else
							CL_Tile.Rot.Y = 0;
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		//region User Input Handler
			...
			$("#cameraBtn").mousedown(function(e){
				_self.toggleCamera();
			});
