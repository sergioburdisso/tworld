...
//region Methods
	//region Public
		...
		this.gameIsOver = function(robs, goal) {
			...
			//PLAYERS STATS:
			for (var i= 1, iMm; i < _NUMBER_OF_ROBS; i++){
				iMm = nextProperty(_i);
			...
			for (var i= 0, iMm=""; i < _NUMBER_OF_ROBS; i++){
			...
			for (iMm in _i){
				...
				if (_i[iMm].$e)
					_i[iMm].$e.html((!/BatteryUse/i.test(iMm)? value : (value/10).toFixed(1)+"%") + "<br>(" + _ROBS[_i[iMm].v].NAME +")");
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		for (var list, img, title, type= 0; type < 2; ++type){
			switch(type){
				case 0:
					list = wonC;
					img = "goal.png";
			...
			if (list.length > 0){
				$goalsTable.append('<tr><th></th><th style="color: rgb(0, 255, 72);">'+title+'</th></tr>');
				for (var desc, value, c= 0; c < list.length; ++c){
					desc = '<tr><td><img id="'+_GAMEOVER[list[c]].$ID+'" src="imgs/'+img+'"/></td><td>';
					switch(list[c]){
						case "TIME":
							value = toMMSS(_GAMEOVER[list[c]].VALUE);
							break;
						case "ROBS_LOCATION":
							var robFinalPos = _ROBS[_GAMEOVER[list[c]].VALUE[0]].FINAL_POSITION;
							if (_GAMEOVER[list[c]].VALUE.length == 1)
								value = sprintf("(%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);
							else
								value = sprintf("el 0 en (%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);

							for (var i=_GAMEOVER[list[c]].VALUE.length-1; i >= 1 ; --i){
								robFinalPos = _ROBS[_GAMEOVER[list[c]].VALUE[i]].FINAL_POSITION;
								value += sprintf(", el %s en (%s,%s)", i, robFinalPos.ROW, robFinalPos.COLUMN);
							}

							desc+= sprintf((_GAMEOVER[list[c]].VALUE.length == 1)?_GAMEOVER[list[c]].$TEXT.SINGULAR:_GAMEOVER[list[c]].$TEXT.PLURAL, value);
							value= "ROBS_LOCATION";
							break;
						case "BATTERY_USE":
							value = _GAMEOVER[list[c]].VALUE/10 + "%";
							break;
						default:
							value = _GAMEOVER[list[c]].VALUE;
					}
					if (value != "ROBS_LOCATION")
						desc+= sprintf(
								(
									(parseInt(value) == 1)?
									_GAMEOVER[list[c]].$TEXT.SINGULAR:
									_GAMEOVER[list[c]].$TEXT.PLURAL
								),
								value
							);
