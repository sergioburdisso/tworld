...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		if (wonC.length > 0){
			...
			for (var desc, value, c= 0; c < wonC.length; ++c){
				desc = '<tr><td><img id="'+_GAMEOVER[wonC[c]].$ID+'" src="imgs/goal.png"/></td><td>';
				switch(wonC[c]){
					case "TIME":
						value = toMMSS(_GAMEOVER[wonC[c]].VALUE);
						break;
					case "ROBS_LOCATION":
						var robFinalPos = _ROBS[_GAMEOVER[wonC[c]].VALUE[0]].FINAL_POSITION;
						if (_GAMEOVER[wonC[c]].VALUE.length == 1)
							value = sprintf("(%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);
						else
							value = sprintf("el 0 en (%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);

						for (var i=_GAMEOVER[wonC[c]].VALUE.length-1; i >= 1 ; --i){
							robFinalPos = _ROBS[_GAMEOVER[wonC[c]].VALUE[i]].FINAL_POSITION;
							value += sprintf(", el %s en (%s,%s)", i, robFinalPos.ROW, robFinalPos.COLUMN);
						}

						desc+= sprintf((_GAMEOVER[wonC[c]].VALUE.length == 1)?_GAMEOVER[wonC[c]].$TEXT.SINGULAR:_GAMEOVER[wonC[c]].$TEXT.PLURAL, value);
						value= "ROBS_LOCATION";
						break;
					case "BATTERY_USE":
						value = _GAMEOVER[wonC[c]].VALUE/10 + "%";
						break;
					default:
						value = _GAMEOVER[wonC[c]].VALUE;
				}
				if (value != "ROBS_LOCATION")
					desc+= sprintf(
							(
								(parseInt(value) == 1)?
								_GAMEOVER[wonC[c]].$TEXT.SINGULAR:
								_GAMEOVER[wonC[c]].$TEXT.PLURAL
							),
							value
						);
		...
		if (lostC.length > 0){
			...
			for (var desc, value, c= 0; c < lostC.length; ++c){
				desc = '<tr><td><img id="'+_GAMEOVER[lostC[c]].$ID+'" src="imgs/go-mark.png"/></td><td>';
				switch(lostC[c]){
					case "TIME":
						value = toMMSS(_GAMEOVER[lostC[c]].VALUE);
						break;
					case "ROBS_LOCATION":
						var robFinalPos = _ROBS[_GAMEOVER[lostC[c]].VALUE[0]].FINAL_POSITION;
						if (_GAMEOVER[lostC[c]].VALUE.length == 1)
							value = sprintf("(%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);
						else
							value = sprintf("el 0 en (%s,%s)", robFinalPos.ROW, robFinalPos.COLUMN);

						for (var i=_GAMEOVER[lostC[c]].VALUE.length-1; i >= 1 ; --i){
							robFinalPos = _ROBS[_GAMEOVER[lostC[c]].VALUE[i]].FINAL_POSITION;
							value += sprintf(", el %s en (%s,%s)", i, robFinalPos.ROW, robFinalPos.COLUMN);
						}

						desc+= sprintf((_GAMEOVER[lostC[c]].VALUE.length == 1)?_GAMEOVER[lostC[c]].$TEXT.SINGULAR:_GAMEOVER[lostC[c]].$TEXT.PLURAL, value);
						value= "ROBS_LOCATION";
						break;
					case "BATTERY_USE":
						value = _GAMEOVER[lostC[c]].VALUE/10 + "%";
						break;
					default:
						value = _GAMEOVER[lostC[c]].VALUE;
				}
				if (value != "ROBS_LOCATION")
					desc+= sprintf(
							(
								(parseInt(value) == 1)?
								_GAMEOVER[lostC[c]].$TEXT.SINGULAR:
								_GAMEOVER[lostC[c]].$TEXT.PLURAL
							),
							value
						);
				$goalsTable.append(desc + "</td></tr>");
		...
		//region User Input Handler
			...
			$("#ctrl-up").mousedown(function(){ _self.RobWalkNorth(0, true) });
			$("#ctrl-down").mousedown(function(){ _self.RobWalkSouth(0, true) });
			$("#ctrl-left").mousedown(function(){ _self.RobWalkWest(0, true) });
			$("#ctrl-right").mousedown(function(){ _self.RobWalkEast(0, true) });
				...
				if (isValidKey(e.keyCode)){
					switch(e.keyCode){
						...
						case 67://C
							...
							break;
						case 70://F
							fullScreen(document.getElementById("tw-root"));
							break;
						case 16://shift
