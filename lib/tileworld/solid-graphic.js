...
function GraphicTileworld(graphicEngine, environment){
	...
	//private:
		...
		var _cameraAnimationflag = false;
...
//region Methods
	...
	//region private
		...
		function display(CL_Scene, currentTime) {
			...
			//UFO animation
			ShipFlyCircle();
			_CLN_Ship.Rot.Y = CL3D.radToDeg(Math.atan2(_CLN_Ship.Pos.X- _CLN_Rob.Pos.X, _CLN_Ship.Pos.Z - _CLN_Rob.Pos.Z));

			//Camera animation
			//if (_Running){
			if (_cameraAnimationflag){
				var _AnimationFactor = 4;
			...
			if (_FollowRob){
				if (_CAMERA_SMOOTH){
					_cameraXPosOffset = (_CLN_Rob.Pos.X - _CL_ActiveCameraTarget.X)/30;
					_cameraZPosOffset = (_CLN_Rob.Pos.Z - _CL_ActiveCameraTarget.Z)/30;
					...
					_CL_ActiveCameraTarget.X = _CLN_Rob.Pos.X;
					_CL_ActiveCameraTarget.Z = _CLN_Rob.Pos.Z;
				}
			}
			...
			//region fps calculation (how many time does it take the browser to render 60 frames?)
				...
				if (this.framesCounter == 0){
					_self.currentFPS = parseInt(_FPS/((currentTime - this.oldCurrentTime)/_FPSTime)); 
					_LaserBeamLifeTime = _self.currentFPS;

					//auto calculate Rob's movements speed according to user's PC performance (current fps)
					_self.Environment.Costs.move = _self.Rob.autoCalculateSpeed(_self.currentFPS);
		...
		this.toggleCamera = function(){
			...
			switch(_ACTIVE_CAMERA){
				...
				case _CAMERA_TYPE.FREE_ROB:
					_cameraTargetFinalPos.setTo(_CLN_Rob.Pos);
					_cameraTargetFinalPos.Y = _CLN_Rob.getBoundingBox().getCenter().Y + 6;
		...
		function createHoleFilledAnimationLight() {
			...
			_CLN_Rob.setLoopMode(true);
			_CLN_Rob.setAnimation("walk");
		}
...
//region Class Constructor Logic -------------------------------------------------------------------------------------> GraphicTileworld Constructor Begin
	...
	_CL_Engine.OnLoadingComplete = function(){
		...
		_CL_CameraFlyCircle.Center.set(0, 16 + Math.random()*100, 0);
		_CL_ActiveCamera.Animators.clear();
		...
		_CLN_Rob = _CL_Scene.getSceneNodeFromName('rob');
		_self.Rob = new GraphicRob(_CLN_Rob, _self);
			...
			$(document).mousewheel(
				function(event, delta){
					...
					if (_CL_ActiveCamera.getAnimators()[0]){
						_newRadius = _CL_CameraModelViewer.Radius - delta*(_CL_CameraModelViewer.Radius)/40;
						if ( 11 <= _newRadius && _newRadius <= 1915){
							_cameraAnimationflag = false;
							_CL_CameraModelViewer.Radius= _newRadius;
							_CL_CameraModelViewer.RotateSpeed = _CL_CameraModelViewer.Radius*6;
						}
					}else
						if (_ACTIVE_CAMERA == _CAMERA_TYPE.PERCEPT){
							_newRadius = _CL_ActiveCamera.Pos.Y - delta;

							if (35 <= _newRadius && _newRadius <= 177)
								_CL_ActiveCamera.Pos.Y = _newRadius;
						}
