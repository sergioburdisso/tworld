...
//returns a random number number (value - r) where r grows when uncertantyFactor grows
function uncertaintyMaker(value, uncertantyFactor){
	return value - parseInt(random(0, value)*uncertantyFactor);
}
...
function List(size){
	...
	this.getInternalArray = function() { return _items;}
}
...
function ListOfHoles(size) {
	...
	//if this method had removed an entire hole then it returns it
...
CL3D.LaserBeam = function(srcP, destP, engine, lifeTime)
{
	...
	this.dispose = function(){
		scene.getRootSceneNode().removeChild(laserHit);
		scene.getRootSceneNode().removeChild(this);
	}
}
