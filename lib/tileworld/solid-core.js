...
*solid-core.js
*<p>
*Tileworld is intended as a framework for the more general
*investigation of intelligent behavior in dynamic environments.
*
*Their version of Tileworld had no tiles, agents would score points by simply
*going to a cell that contained a hole. Even with this simplified version of Tileworld
*the resulting... [A history of TW]
*<p>
...
//region "Knobs" (The knob settings control the evolution of a Tileworld simulation)
	...
	Tileworld.HoleSize = 3;
	Tileworld.HolesSize_UncertaintyThreshold = 1;
	Tileworld.NumberOfHoles = 4; // how many holes are going to appear each time?
	Tileworld.NumberOfHoles_UncertaintyThreshold = 0.5;
	...
	Tileworld.Dynamism = 20; // time in seconds (the rate at which new holes appear and disappear)
	Tileworld.Dynamism_UncertaintyThreshold = 1; //P(HoleTimeout= Dynamism) = 1/(Floor((Dynamism-1)*p) + 1)

	Tileworld.Hostility = 20; // time in seconds (the rate at which obstacles appear)
	...
	//-> Perception
		...
		Tileworld.FullyObservableGrid = false; // fully observable grid or partially observable grid? 
		Tileworld.VisibilityRadius = 3; // in case the grid is partially observable, it specifies the maximum distance at which any object can be seen (in cells)

		Tileworld.ShowTimeLeft = false; // show holes and obstacles time remaining to timeout

	//--------------------------------------------------------------------------------------> utilityFillCell
	Tileworld.valueOfCellHoleFilled = function(sizeSoFar) {return 3;}// menor que 10 (que el valor de tapar uno de uno completo)
	...
	//region Methods
		...
		//region Private Methods
			...
			function createHole(holeSize, holeLifetime){
				...
				_environment.newHole(holeCells, holeLifetime + _TELEPORT_DELAY, 1-Math.random()*Tileworld.VariabilityOfUtility);
				...
				while ( !stack.empty() ) {
					...
					if ( successors ) {
						for (var i= 0, suc; i < successors.length; i++){
							...
							if (getManhattanDistance(suc, rootCell) <= tileDistance && !alreadyTested.isIn(suc[0], suc[1]))
