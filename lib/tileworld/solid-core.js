			...
			*/
			this.perceptionFunction = function(environment) /*returns a JSON*/{ // VER SI NO LO METO EN UN WORKER QUE LLAME A OTRO WORKER (PROGRAMA AGENTE)
				...
				if (_fullyObservable){
					...
					this.Percept.extra.agentLocation[0] = environment.RobLocation.Row;
					this.Percept.extra.agentLocation[1] = environment.RobLocation.Column;

				}else{
					// if environment is partially observable
					// visibility grid
					for (var rVOEnvGrid, totalRows= this.Percept.grid.length, r= 0; r < totalRows; ++r)
						for (var  cVOEnvGrid, totalColumns= this.Percept.grid[0].length, c= 0; c < totalColumns; ++c){
							rVOEnvGrid = environment.RobLocation.Row - _visibilityRadius; //VO stands for Virtual Origin
							cVOEnvGrid = environment.RobLocation.Column - _visibilityRadius; //VO stands for Virtual Origin
				...
				if (_fullyObservable){
					...
					for (var iHole, iHCells, i= 0;  i < _totalHoles; ++i){
						iHole = environment.ListOfHoles.getItemAt(i);
						/*for each current cell si "es visible" a agrego en currentCellX de la perception*/
						if(true/*currentCellX al menos tiene una celda*/){
							...
							this.Percept.extra.listOfHoles[vi].cells = iHole.CurrentCells.getInternalArray();
							this.Percept.extra.listOfHoles[vi].size = iHole.Size;
				...
				for (var tileCounter= 0, rTile, r= 0; r < this.Percept.grid.length; ++r)
					for (var cTile, c= 0; c < this.Percept.grid[0].length; ++c)
						if (this.Percept.grid[r][c] == GRID_CELL.Tile){
							...
							cTile = (_fullyObservable)? c : _visibilityRadius - c;
